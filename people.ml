...
and (* class *) infected (initx : int) (inity : int) =
  object (self)
    ...
    (* add mutable value representing time left to recovery *)
    val mutable time_steps = Utilities.gaussian (fst cRECOVERY_PERIOD) (snd cRECOVERY_PERIOD)

    (* problem 205 - draw method *)
    method! draw = 
      let x, y = self#pos in 
      Viz.draw_circle x y cCOLOR_INFECTED

    (* problem 206/208 - update method *)
    method! update = 
      super#update; 
      let posx, posy = self#pos in 
